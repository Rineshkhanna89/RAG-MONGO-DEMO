...
Donwload and install the node.js (https://nodejs.org/en/download)
even then if we npm install error in vs code terminal
Run the powershell as 'admin' and run the below cmd
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope LocalMachine
               or
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
 Now npm install will work in vs code

Steps to implement

1) extract the zip
2) open the vscode and add the folder to your workspace
3) open the terminal --> go to rag-mongo-demo-v7  --> npm install
4) open the .env and read the inputs an
...
7) Start the server (web - client and backend - server)
    7.1) npm run dev
	Front app starts at this http://localhost:3000/
